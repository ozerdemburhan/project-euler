...
const HANDS1 = [
    ...
    // "TH JH QH KH AH 2C 3H 4D 5C 6S",
    // "5D 6D 7D 8D 9D 2C 3H 4D 5C 6S",
    // "3D 3S 3C 2H 2S 2C 3H 4D 5C 6S",
];

const PLAYER1_WINS = 0;
const PLAYER2_WINS = 1;
const NO_WINNER = 2;

function compareValues(values1, values2) {
    var arr1 = values1.slice();
    var arr2 = values2.slice();
    arr1.reverse();
    arr2.reverse();

    var ubound = Math.min(arr1.length, arr2.length);
    for(var i = 0; i < ubound; i++) {
        var index1 = VALUES.indexOf(arr1[i]);
        var index2 = VALUES.indexOf(arr2[i]);
        if(index1 > index2) {
            return PLAYER1_WINS;
...
function compareForHighestCard(hand1, hand2) {
    var value1 = getHighestValue(hand1);
    var value2 = getHighestValue(hand2);
...
function xor(result1, result2) {
    ...
    var c = compareValues(result1[1], result2[1]);
    if(c == NO_WINNER) {
        if(result1[2] && result2[2]) {
            return compareValues(result1[2], result2[2]);
        }
    }
...
function compareForOnePair(hand1, hand2) {
    var hasOnePair1 = hasOnePair(hand1);
    var hasOnePair2 = hasOnePair(hand2);

    return xor(hasOnePair1, hasOnePair2);
}
...
function hasOnePair(hand) {
    ...
    if(pairs.length == 1) {
        return [true, pairs, kickers];
    }

    return [false];
}

function compareForTwoPairs(hand1, hand2) {
    var hasTwoPair1 = hasTwoPairs(hand1);
    var hasTwoPair2 = hasTwoPairs(hand2);

    return xor(hasTwoPair1, hasTwoPair2);
}
...
function compareForThreeOfAKind(hand1, hand2) {
    var hasTriple1 = hasThreeOfAKind(hand1);
    var hasTriple2 = hasThreeOfAKind(hand2);

    return xor(hasTriple1, hasTriple2);
}

function hasThreeOfAKind(hand) {
    var values = hand["values"];
    var arr = Object.keys(values);

    var tofak;
    var kickers = [];

    for(var i = 0; i < arr.length; i++) {
        var valueArr = values[arr[i]];
    ...
    if(tofak) {
        return [true, tofak, kickers];
    }

    return [false];
}

function compareForStraight(hand1, hand2) {
    var hasStraight1 = hasStraight(hand1);
    var hasStraight2 = hasStraight(hand2);

    return xor(hasStraight1, hasStraight2);
}
...
function hasStraight(hand) {
    ...
    for(var suit in suits) {
        var suitValues = suits[suit];
        for(var i = 0; i < suitValues.length; i++) {
            values.push(suitValues[i]);
        }
    }

    sortValues(values);

    var valueMin = VALUES.indexOf(values[0]);
    var valueMax = VALUES.indexOf(values[values.length - 1]);
    if(valueMax - valueMin == values.length - 1) {
        return [true, values];
    }
...
function compareForFlush(hand1, hand2) {
    var hasFlush1 = hasFlush(hand1);
    var hasFlush2 = hasFlush(hand2);

    return xor(hasFlush1, hasFlush2);
}


function hasFlush(hand) {
    var suits = hand["suits"];
    if(getObjectSize(suits) == 1) {
        var suit = Object.keys(suits)[0];
        return [true, suits[suit]];
    }

    return [false];
}

function compareForFullHouse(hand1, hand2) {
    var hasFullHouse1 = hasFullHouse(hand1);
    var hasFullHouse2 = hasFullHouse(hand2);

    return xor(hasFullHouse1, hasFullHouse2);
}

function hasFullHouse(hand) {
    var values = hand["values"];
    if(getObjectSize(values) != 2) {
        return [false];
    }
    ...
    for(value in values) {
        var arr = values[value];

        if(arr.length == 3) {
            if(!threeofak) {
                threeofak = [];
            }
            threeofak.push(value);
    ...
    if(threeofak && twoofak) {
        return [true, threeofak, twoofak];
    }

    return [false];
}

function compareForFourOfAKind(hand1, hand2) {
    var hasFourOfAKind1 = hasFourOfAKind(hand1);
    var hasFourOfAKind2 = hasFourOfAKind(hand2);

    return xor(hasFourOfAKind1, hasFourOfAKind2);
}
...
function compareForStraightFlush(hand1, hand2) {
    var hasStraightFlush1 = hasStraightFlush(hand1);
    var hasStraightFlush2 = hasStraightFlush(hand2);

    return xor(hasStraightFlush1, hasStraightFlush2);
}
...
function hasStraightFlush(hand) {
    ...
    if(getObjectSize(values) != 5) {
        return [false];
    }

    for(var suit in suits) {
        var values = suits[suit];
        var start = VALUES.indexOf(values[0]);
        var end = VALUES.indexOf(values[values.length - 1]);
